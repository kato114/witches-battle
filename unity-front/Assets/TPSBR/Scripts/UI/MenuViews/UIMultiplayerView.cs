...
namespace TPSBR.UI
{
	public class UIMultiplayerView : UICloseView
	{
		...
		[SerializeField] private UIButton _buybulletButton;
		...
		[SerializeField] private TextMeshProUGUI _coinBalance;
		...
		protected override void OnInitialize()
		{
			...
			_buybulletButton.onClick.AddListener(OnBuyBulletButton);
		...
		protected override void OnOpen()
		{
			...
			int coin = Global.PlayerService.PlayerInfo.coin;
			Debug.Log("Balance : " + coin);
			_coinBalance.text = coin.ToString();

			Context.PlayerData.Nickname = Global.PlayerService.PlayerInfo.name;
		}
		...
		private void OnCreateGameButton()
		{
			...
			else
			{
				...
				if (currCoin >= 20)
				{
					Global.PlayerService.PlayerInfo.coin -= 20;
					Open<UICreateSessionView>();
                }
                else
                {
					var infoDialog = Open<UIErrorDialogView>();
					infoDialog.Title.text = "Faild Create Room";
					infoDialog.Description.text = "Your score is not enough to create Room. Please charge your score. please click \"Buy Bullet\" to buy bullet";
				}
			}
		}
		...
		private async void OnQuickPlayButton()
		{
			...
			{				
                ...
                else
                {
					...
					infoDialog.Title.text = "Faild Join Room";
					infoDialog.Description.text = "Your score is not enough to join room. Please charge your score.";
				}				
			}
			else
			{
		...
		private void OnBuyBulletButton()
        {
		...
		IEnumerator ConnectServer()
        {
			...
			while (true)
			{
				...
				if (message != null)
				{
                    ...
                    {
						Debug.Log("Balance : " + replyObject.data);
						string balance = replyObject.data;
						_coinBalance.text = balance.Trim();
						Global.PlayerService.PlayerInfo.coin = int.Parse(balance.Trim());
					}
				}
